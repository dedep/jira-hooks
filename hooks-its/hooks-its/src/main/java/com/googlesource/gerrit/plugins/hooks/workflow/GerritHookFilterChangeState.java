...
public class GerritHookFilterChangeState extends GerritHookFilter {
  ...
  private void performAction(ChangeAttribute change, Condition... conditionArgs)
    ...
    TransitionsProfile profile = its.loadProfile(issues);

    Transition transition = null;
    List<Transition> transitions = loadTransitions(profile);
  ...
  private List<Transition> loadTransitions(TransitionsProfile profile) {
    File configFile = new File(sitePath, profile.getProfileFilePath());
    FileBasedConfig cfg = new FileBasedConfig(configFile, FS.DETECTED);
    try {
      cfg.load();
      log.debug("Transitions loaded from following file: " + cfg.toString());
    } catch (IOException e) {
